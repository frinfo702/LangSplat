...
def eval_gt_lerfdata(
    json_folder: Union[str, Path] = None, ouput_path: Path = None
) -> Dict:
    """
...
def activate_stream(
    ...
    for k in range(n_prompt):
        ...
        for i in range(n_head):
            ...
            output_path_relev = (
                image_name / "heatmap" / f"{clip_model.positives[k]}_{i}"
            )
            output_path_relev.parent.mkdir(exist_ok=True, parents=True)
            colormap_saving(
                valid_map[i][k].unsqueeze(-1), colormap_options, output_path_relev
            )
            ...
            output_path_compo = (
                image_name / "composited" / f"{clip_model.positives[k]}_{i}"
            )
...
def evaluate(
    ...
    gt_ann, image_shape, image_paths = eval_gt_lerfdata(
        Path(json_folder), Path(output_path)
    )
    eval_index_list = [int(idx) for idx in list(gt_ann.keys())]
    compressed_sem_feats = np.zeros(
        (len(feat_dir), len(eval_index_list), *image_shape, 3), dtype=np.float32
    )
    ...
    for j, idx in enumerate(tqdm(eval_index_list)):
        ...
        acc_num_img = lerf_localization(
            restored_feat, rgb_img, clip_model, image_name, img_ann
        )
        acc_num += acc_num_img
...
if __name__ == "__main__":
    ...
    feat_dir = [
        os.path.join(
            args.feat_dir, dataset_name + f"_{i}", "train/ours_None/renders_npy"
        )
        for i in range(1, 4)
    ]
